...
def create_shell(data):
    ...
    Create Shell
    """
    query = "INSERT INTO shells(shell_id, vision, thought, tag_name, timestamp) VALUES(?, ?, ?, ?, ?);"
    cur = conn.cursor()
...
def list_shells():
    ...
    List All Shells
    """
    query = "SELECT shell_id, vision, thought, tag_name, timestamp FROM shells;"
    cur = conn.cursor()
    cur.execute(query)
    data = []
    for row in cur:
...
def get_shell_by_offset(offset):
    ...
    """
    query = f'SELECT shell_id, vision, thought, tag_name, timestamp FROM shells LIMIT 1 OFFSET {str(offset)};'
    cur = conn.cursor()
    cur.execute(query)
    data = None
    for row in cur:
...
def get_shell_from_ids(id_list=None):
    ...
    query = f'SELECT shell_id, vision, thought, tag_name, timestamp \
...
def get_shell_from_id(id=''):
    ...
    Get shell from id_list
    """
    query = f'SELECT shell_id, vision, thought, tag_name, timestamp FROM shells WHERE shell_id="{id}";'
    cur = conn.cursor()
    cur.execute(query)
    data = None
    for row in cur:
